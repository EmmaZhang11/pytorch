...
static IntArrayRef get_size_for_index(const Tensor& tensor, int i)
{
  if (tensor.is_nested())
  {
    std::vector<IntArrayRef> tensor_sizes = NestedTensor_get_sizes(get_nested_tensor_impl(tensor));
...
static IntArrayRef get_stride_for_index(const Tensor& tensor, int i)
{
  if (tensor.is_nested())
  {
    std::vector<IntArrayRef> tensor_strides = NestedTensor_get_strides(get_nested_tensor_impl(tensor));
...
static int64_t get_offset_for_index(const Tensor& tensor, int i)
{
  if (tensor.is_nested())
  {
    int64_t* offsets_ptr = get_nested_tensor_impl(tensor)->get_storage_offsets().data_ptr<int64_t>();
